...
const api = {
  formatter: {//渲染的方法
      ...
      custom: function (value, row, index) {
          ...
          const url = fixurl('example/bootstraptable/change')
          return '<a class="btn-change text-success" data-url="'+url+'" data-id="' + row.id + '"><i class="fa ' + (row.title == '' ? 'fa-toggle-on fa-flip-horizontal text-gray' : 'fa-toggle-on') + ' fa-2x"></i></a>';
      }
  },
  events: {//绑定事件的方法
    ip: {
...
export function index() {
  ...
  Table.api.init({
      extend: {
          index_url: fixurl('example/bootstraptable/index'),
          add_url: '',
          edit_url: '',
          del_url: fixurl('example/bootstraptable/del'),
          multi_url: '',
      }
  });
  ...
  table.on('post-common-search.bs.table', function (event, table) {
      ...
      $("input[name='title']", form).addClass("selectpage").data("source", fixurl("auth/adminlog/selectpage")).data("primaryKey", "title").data("field", "title").data("orderBy", "id desc");
      $("input[name='username']", form).addClass("selectpage").data("source", fixurl("auth/admin/index")).data("primaryKey", "username").data("field", "username").data("orderBy", "id desc");
      ...
      columns: [
        [
            ...
            //通过Ajax渲染searchList，也可以使用JSON数据
            {
                ...
                title: __('Url'),
                align: 'left',
                searchList: $.getJSON(fixurl('example/bootstraptable/searchlist')+'?search=a&field=row[user_id]'),
                formatter: api.formatter.url
            },
            //点击IP时同时执行搜索此IP
            {
                field: 'ip',
                title: __('IP'),
                events: api.events.ip,
                formatter: api.formatter.ip
            },
            //自定义栏位,custom是不存在的字段
            {field: 'custom', title: __('切换'), operate: false, formatter: api.formatter.custom},
            //browser是一个不存在的字段
            //通过formatter来渲染数据,同时为它添加上事件
            {
                ...
                title: __('Browser'),
                operate: false,
                events: api.events.browser,
                formatter: api.formatter.browser
            },
            {
                field: 'admin_id', 
                title: __('联动搜索'), 
...
export function detail() {
  $(document).on('click', '.btn-callback', function () {
    ModalLayer.close($("input[name=callback]").val());
  });
}