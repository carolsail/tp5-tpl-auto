    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $params['avatar'] = '/static/img/avatar.png'; //设置新管理员默认头像。
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                //这里修改validate规则，主要让password可以为空
                $validate = new \app\admin\validate\Admin;
                $validate->rule([
                  'password' => 'regex:\S{32}'
                ]);
    ...
    public function del($ids = "")
    {
        if ($ids) {
            ...
            $adminList = $this->model->where('id', 'in', $ids)->where('id', 'in', function ($query) use ($childrenGroupIds) {
                $query->table('auth_group_access')->where('group_id', 'in', $childrenGroupIds)->field('uid');
            })->select();
            if ($adminList) {
                $deleteIds = [];
